	...
	<div id="main_content">
		...
		<div style="width:90%;margin-left:50px;font-size:14px">	
				...
				<center>
					...
					<h3>LIST OF PASSERS FOR PRE-EMPLOYMENT AND PROMOTIONAL TEST</h3>
				...
				<?php 
					...
					foreach ($prom_exams as $prom): 
					
					$prom_passers = $controller->get_list_passers($prom->exam_type,$prom->exam_bracket);
					
					if(!empty($prom_passers)):
					
					
					 echo "<h4>SG ".$prom->exam_bracket."</h4>";
					
					$no=1;
					foreach ($prom_passers as $row): 
				?>
				...
				<?php $no++; endforeach; endif; endforeach; ?>	
				...
				<?php 
					...
					foreach ($prem_exams as $prem): 
					
					$prem_passers = $controller->get_list_passers($prem->exam_type,$prem->exam_bracket);
					
					if(!empty($prem_passers)):
					
					
					echo "<h4>SG ".$prem->exam_bracket."</h4>";
					
					$no=1;
					foreach ($prem_passers as $row): 
				?>
